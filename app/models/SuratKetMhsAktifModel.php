...
class SuratKetMhsAktifModel extends Database 
{

    private $table = 'surat_keterangan_mhs_aktif';
    ...
    public function insertSurat($data)
    {
        ...
        ('', :email, :tahun_sekarang, :nama, :tempat_lahir, :tanggal_lahir, :alamat_desa_kel, :alamat_kec_kota, :fakultas_id, :program_studi_id, :nim, :tahun_akademik, :semester, :kegunaan , :created_at)
        ";
    ...
    public function updateSurat($id, $data)
    {
        ...
        email=:email, tahun_sekarang=:tahun_sekarang, nama=:nama, tempat_lahir=:tempat_lahir, tanggal_lahir=:tanggal_lahir, alamat_desa_kel=:alamat_desa_kel, alamat_kec_kota=:alamat_kec_kota, fakultas_id=:fakultas_id, program_studi_id=:program_studi_id, nim=:nim, tahun_akademik=:tahun_akademik, semester=:semester, kegunaan=:kegunaan WHERE id=:id
        ";
    ...
    public function getBySearching($post)
    {
        ...
        LEFT JOIN fakultas f ON s.fakultas_id = f.id
        LEFT JOIN program_studies ps ON s.program_studi_id = ps.id
        WHERE s.nim LIKE :nim OR s.nama LIKE :nama OR s.kegunaan LIKE :kegunaan OR ps.program_studi LIKE :program_studi OR s.tahun_akademik LIKE :tahun_akademik
        ";

        $this->query($query);
        $this->bind('nim', "%$key%");
        $this->bind('nama', "%$key%");
        $this->bind('kegunaan', "%$key%");
